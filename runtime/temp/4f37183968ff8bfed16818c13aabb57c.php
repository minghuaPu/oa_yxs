<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:76:"D:\wampserver\wamp64\www\oa\public/../application/admin\view\bell\index.html";i:1546998629;s:69:"D:\wampserver\wamp64\www\oa\public/../application/admin\view\top.html";i:1546998629;s:70:"D:\wampserver\wamp64\www\oa\public/../application/admin\view\left.html";i:1547019384;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<style>
	.holiday {border: 1px solid #bbb;width: 75%;margin:20px auto;overflow: hidden;}
	 .holiday_box{width: 100%;display: flex;justify-content: space-around;margin:10px 0;}
	.tijiao{float: right;margin:10px 70px;}
    .box_kuang{display: flex;}
    .box_kuang div{width: 100px;line-height: 35px;}
</style>
<div class="container" style="height: 100vh;">
	<?php if($user_data['user_cate']=='员工'): ?>
	 <form action="<?php echo url('holiday'); ?>">
	     <div class="holiday">
	     	<h3 style="text-align: center;">调休申请</h3>
			<div  class="holiday_box">
				...
				<textarea class="form-control" style="width: 83%;margin:auto;" placeholder="调休原因..." name="reason"></textarea>
				<button class="btn btn-success tijiao">提交申请</button>
			</div>
	</form>
	<?php endif; ?>
<table class="table table-striped table-bordered table-hover table-condensed" style="margin-top: 50px;">
			<thead>
				<tr>
					<th style="text-align: center!important;" width="15%">姓名</th>
			...
			<tbody>	
				<?php if($user_data['user_cate']=='员工'): foreach($list as $key=>$val): ?>
				<tr style="text-align: center;">
					...
					<?php if($val['state']==0): ?>
					<td>审核中。。</td>
					<?php endif; if($val['state']==1): ?>
					<td  style="padding: 12px;color: #4cae4c;">同意</td>
					<?php endif; if($val['state']==2): ?>
					<td style="padding: 12px;color: #d43f3a;">拒绝</td>
					<?php endif; ?>
				</tr>
				<?php endforeach; endif; if($user_data['user_cate']=='老板'): foreach($list as $key=>$val): ?>
				<tr style="text-align: center;">
					<td style="padding: 12px;"><?php echo $val['user_name']; ?></td>
					<td style="padding: 12px;"><?php echo $val['start_time']; ?></td>
					<td style="padding: 12px;"><?php echo $val['end_time']; ?></td>
					<td style="padding: 12px;"><?php echo $val['reason']; ?></td>
					<?php if($val['state']==0): ?>
					<td>
						<a href="<?php echo url('agree',['id'=>$val['id']]); ?>" style="margin-right: 10px;" class="btn btn-success">同意</a>
						<a href="<?php echo url('reject',['id'=>$val['id']]); ?>" class="btn btn-danger">拒绝</a>
					</td>
					<?php endif; if($val['state']==1): ?>
					<td style="padding: 12px;color: #4cae4c;">同意</td>
					<?php endif; if($val['state']==2): ?>
